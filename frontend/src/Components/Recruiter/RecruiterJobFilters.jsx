...
const RecruiterJobFilters = () => {
  ...
  const [salary, setSalary] = useState("");
    ...
    const [candidateJobLevel, setCandidateJobLevel] = useState("");
      ...
      <div className="max-w-7xl mx-auto px-4 py-4">
        ...
        <div className="w-full bg-white grid grid-cols-1 sm:grid-cols-5 gap-3 p-4 rounded-sm shadow-sm">
          ...
          <button className="bg-[#045fc7] text-white rounded-md text-sm hover:bg-[#034da3] transition">
            ...
            {isAdvanced && (
                <div className="sm:col-span-5 grid grid-cols-1 sm:grid-cols-5 gap-6 p-4 text-gray-600">
                    ...
                    {/* Job Level (Radio) */}
                    <div className="flex flex-col gap-2">
                    <p className="font-semibold mb-2">Candidate Level</p>
                    {jobFilterData.candidateLevel.map((item) => (
                        <label key={item.value} className="flex items-center gap-2 text-sm cursor-pointer">
                        <input
                            type="radio"
                            name="jobLevel"
                            value={item.value}
                            checked={candidateJobLevel === item.value}
                            onChange={(e) => setCandidateJobLevel(e.target.value)}
                            className="accent-[#0A65CC]"
                        />
                        <span className={candidateJobLevel === item.value ? "text-[#0A65CC]" : ""}>
                            {item.label}
                        </span>
                        </label>
                    ))}
