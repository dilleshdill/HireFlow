...
const FindCandidateCard = () => {
  ...
  useEffect(() => {
    const fetchCandidates = async () => {
      try {
        const response = await axios.get(
          DOMAIN + `/api/job/get-candidates/${id}`,
    ...
    const fetchSavedCandidates = async () => {
      try {
        const response = await axios.get(
          DOMAIN + "/api/recruiter/get-all-savedCandidates",
          { withCredentials: true },
        );
        if (response.status === 200) {
  ...
  const addToSaved = async (userId) => {
    try {
      const response = await axios.post(
        DOMAIN + "/api/recruiter/save-candidate",
        { userId },
        { withCredentials: true },
      );
      if (response.status === 200) {
  ...
  const removeFromSaved = async (userId) => {
    try {
      const response = await axios.post(
        DOMAIN + "/api/recruiter/remove-candidate",
        { userId },
        { withCredentials: true },
      );
      if (response.status === 200) {
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center justify-between sm:justify-end gap-4 sm:flex-shrink-0">
                {savedCandidates.some(
                  (s) =>
                    s &&
                    String(s.userId.userId._id) ===
                      String(item.profile.userId._id),
                ) ? (
                  <button
                    onClick={() => {
                      (setCurDeleteId(item.profile._id), setShowDelete(true));
                    }}
                  ...
                  <button
                    onClick={() => {
                      (setCurAddedId(item.profile._id), setShowAdded(true));
                    }}
                  >
                ...
                <button
                  onClick={() => navigate(`/user/job-page/${item.profile._id}`)}
                  className="bg-blue-100 px-4 py-2 flex text-md font-medium items-center gap-2 text-blue-500 rounded-md hover:bg-blue-600 hover:text-white transition"
                >
                  View Profile
